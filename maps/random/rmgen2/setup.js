...
var g_PlayerbaseTypes = {
	"line": {
		"getPosition": (distance, groupedDistance, startAngle) =>
			placeLine(getTeamsArray(), distance, groupedDistance, startAngle),
	...
	"radial": {
		"getPosition": (distance, groupedDistance, startAngle) =>
			playerPlacementCircle(distance, startAngle),
...
function pickAmount(amounts)
{
	...
	if (amount in g_Amounts) return g_Amounts[amount];
...
function pickMix(mixes)
{
	...
	if (mix in g_Mixes) return g_Mixes[mix];
...
function pickSize(sizes)
{
	...
	if (size in g_Sizes) return g_Sizes[size];
...
{
	return createBases(
		...g_PlayerbaseTypes[type].getPosition(
			distance,
			groupedDistance,
			startAngle
		),
		g_PlayerbaseTypes[type].walls
	);
}
...
function createBase(playerID, playerPosition, walls)
{
	placePlayerBase({
		...
		"baseResourceConstraint": avoidClasses(
			g_TileClasses.water,
			0,
		...
		"CityPatch": {
			...
			"painters": [new TileClassPainter(g_TileClasses.player)],
		},
		"StartingAnimal": {
...
function getTeamsArray()
{
	...
	for (let i = 0; i < numPlayers; ++i)
		if (getPlayerTeam(playerIDs[i]) == -1) teams.push([playerIDs[i]]);
...
function placeLine(teamsArray, distance, groupedDistance, startAngle)
{
	...
	for (let i = 0; i < teamsArray.length; ++i)
	{
		...
		var teamAngle =
            startAngle + ((i + 1) * 2 * Math.PI) / teamsArray.length;
		...
		for (let p = 0; p < teamsArray[i].length; ++p)
		{
			...
			playerPosition.push(
				Vector2D.add(
					mapCenter,
					new Vector2D(safeDist + p * groupedDistance, 0).rotate(
						-teamAngle
					)
				).round()
			);
...
function placeStronghold(teamsArray, distance, groupedDistance, startAngle)
{
	...
	for (let i = 0; i < teamsArray.length; ++i)
	{
		var teamAngle =
            startAngle + ((i + 1) * 2 * Math.PI) / teamsArray.length;
		var teamPosition = Vector2D.add(
			mapCenter,
			new Vector2D(distance, 0).rotate(-teamAngle)
		);
		...
		{
			var angle =
                startAngle + ((p + 1) * 2 * Math.PI) / teamsArray[i].length;
			playerIDs.push(teamsArray[i][p]);
			playerPosition.push(
				Vector2D.add(
					teamPosition,
					new Vector2D(teamGroupDistance, 0).rotate(-angle)
				).round()
			);
		}
	}
...
function initTileClasses(newClasses)
{
	...
	if (newClasses) classNames = classNames.concat(newClasses);
