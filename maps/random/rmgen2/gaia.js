...
function createBluffsPassages(playerPosition)
{
	...
	for (const position of playerPosition)
	{
		...
		{
			const angle =
                position.angleTo(g_Map.getCenter()) +
                (randFloat(-1, 1) * Math.PI) / 2;
			const start = Vector2D.add(
				position,
				new Vector2D(defaultPlayerBaseRadius() * 0.7, 0)
					.rotate(angle)
					.perpendicular()
			).round();
			const end = Vector2D.add(
				position,
				new Vector2D(defaultPlayerBaseRadius() * randFloat(1.7, 2), 0)
					.rotate(angle)
					.perpendicular()
			).round();

			if (
				g_TileClasses.forest.has(end) ||
                !stayClasses(g_TileClasses.bluff, 12).allows(end)
			)
				continue;

			if (
				(g_Map.getHeight(end.clone().floor()) -
                    g_Map.getHeight(start.clone().floor())) /
...
function addBluffs(constraint, size, deviation, fill, baseHeight)
{
	...
	if (currentBiome() == "generic/india") constrastTerrain = g_Terrains.dirt;
	...
	for (let i = 0; i < fill * 15; ++i)
	{
		...
		if (!areasBluff.length || !areasBluff[0].getPoints().length) continue;
		...
		createArea(
			...
			[
				new LayeredPainter(
					[g_Terrains.mainTerrain, constrastTerrain],
					[5]
				),
				new SmoothElevationPainter(
					ELEVATION_MODIFY,
					elevation * bluffDeviation,
					2
				),
		...
		const slopeLength =
            (1 - margin) *
            Vector2D.average([baseLine.start, baseLine.end]).distanceTo(
            	Vector2D.average([endLine.start, endLine.end])
            );
		...
		{
			const dist = Math.abs(
				distanceOfPointFromLine(baseLine.start, baseLine.end, point)
			);
			g_Map.setHeight(
				point,
				Math.max(
					g_Map.getHeight(point) * (1 - dist / slopeLength) - 2,
					baseHeight
				)
			);
		}
...
function addDecoration(constraint, size, deviation, fill)
{
	...
	var baseCount = 1;
	if (currentBiome() == "generic/india") baseCount = 8;
...
function addElevation(constraint, el)
{
	...
	if (el.class == g_TileClasses.water) elType = ELEVATION_SET;

	var widths = [];

	// Allow for shore and cliff rendering
	for (var s = el.painter.length; s > 2; --s) widths.push(1);
	...
	for (var i = 0; i < count; ++i)
	{
		...
		pElevation = Math.max(
			el.minElevation,
			Math.min(pElevation, el.maxElevation)
		);
...
function addLakes(constraint, size, deviation, fill)
{
	...
	if (
		currentBiome() == "generic/temperate" ||
        currentBiome() == "generic/india"
	)
		lakeTile = g_Terrains.dirt;

	if (currentBiome() == "generic/aegean") lakeTile = g_Terrains.tier2Terrain;

	if (currentBiome() == "generic/autumn") lakeTile = g_Terrains.shore;
	...
	var group = new SimpleGroup(
		[new SimpleObject(g_Decoratives.rockMedium, 1, 3, 1, 3)],
		true,
		g_TileClasses.dirt
	);
	createObjectGroupsDeprecated(
		...
		[
			...
			borderClasses(g_TileClasses.water, 4, 3),
		],
		1000,
		100
	);
	...
	group = new SimpleGroup(
		[
			new SimpleObject(g_Decoratives.reeds, 10, 15, 1, 3),
			new SimpleObject(g_Decoratives.rockMedium, 1, 3, 1, 3),
		],
		true,
		g_TileClasses.dirt
...
function addLayeredPatches(constraint, size, deviation, fill)
{
	...
	for (const patchSize of patchSizes)
	{
		...
		createAreas(
			new ChainPlacer(
				Math.min(patchMinRadius, patchMaxRadius),
				patchMaxRadius,
				Math.floor(patchSize * offset),
				0.5
			),
			[
				new LayeredPainter(
...
function addPlateaus(constraint, size, deviation, fill)
{
	...
	if (
		currentBiome() == "generic/alpine" ||
        currentBiome() == "generic/savanna"
	)
...
function addValleys(constraint, size, deviation, fill, baseHeight)
{
	...
	if (currentBiome() == "generic/india") valleySlope = g_Terrains.dirt;
...
{
	if (currentBiome() == "generic/savanna") return;
...
function addStragglerTrees(constraint, size, deviation, fill)
{
	...
	for (var i = 0; i < trees.length; ++i)
	{
		...
		if (
			i == 2 &&
            (currentBiome() == "generic/sahara" ||
                currentBiome() == "generic/aegean")
		)
			treesMax = 1;
		...
		var group = new SimpleGroup(
			[new SimpleObject(trees[i], min, treesMax, minDist, maxDist)],
			true,
			g_TileClasses.forest
		);
