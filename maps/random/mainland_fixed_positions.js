...
const playerPlacements = playerPlacementCircle(
	fractionToTiles(playerDistanceFraction + numPlayers * 0.007)
);
const [playersOrder, playerPositions, playerAngles] = playerPlacements;

const playerPositionBalanced = playerPlacements;
JSON.stringify(playerPositions);

const playerIDs = [];
for (let i = 0; i < numPlayers; ++i) playerIDs.push(i + 1);
...
placePlayerBases({
	...
	"Mines": {
		"types": [{ "template": oMetalLarge }, { "template": oStoneLarge }],
	},
	"Trees": {
		"template": oTree1,
		"count": 5,
...
if (currentBiome() == "generic/india") planetm = 8;
...
if (!isNomad())
{
	const playerAreas = getSurroundingAreas(playerPositions);
	placePlayerFoodBalanced(
		playerAreas,
		oFruitBush,
		null,
		oMainHuntableAnimal,
