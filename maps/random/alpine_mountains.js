...
if (g_MapSettings.Biome) setSelectedBiome();
else setBiome("alpine_mountains/winter");
...
const pForestMixed = [
	...
	tForestfloor + TERRAIN_SEPARATOR + oFirA,
	tForestfloor,
];
const pForestFir = [
	tForestfloor + TERRAIN_SEPARATOR + oFirA,
	tForestfloor + TERRAIN_SEPARATOR + oFirB,
	tForestfloor,
];
const pForestPineSnow = [
	tForestfloorSnow + TERRAIN_SEPARATOR + oPineSnow,
	tForestfloorSnow,
];

const heightLand = 30;
const heightOffsetBump = 2;
const snowSlopeThreshold =
    currentBiome() == "alpine_mountains/summer" ? 1.7 : 2.3;
...
createArea(
	new DiskPlacer(mapSize / 2 - 8, new Vector2D(mapSize / 2, mapSize / 2)),
	new TileClassPainter(clNonBordering),
	new NullConstraint()
);
...
while (true)
{
	...
	mountainBorderArea = createArea(
		new MapBoundsPlacer(),
		null,
		borderClasses(clMountain, 0, 8)
	);
	...
	if (!isNomad())
	{
		for (let i = 0; i < numPlayers; ++i)
		{
			surroundingPlayersAreas[i] = createArea(
				new DiskPlacer(surroundingPlayerAreaMax, playerPosition[i]),
				null,
				avoidClasses(clPlayer, surroundingPlayerAreaMin)
			);
		}
	}
	...
	for (const forestVariant of forestVariants)
		createAreasInAreas(
			...
			new AndConstraint([
				...
				borderClasses(clMountain, 2, 30),
				new SlopeConstraint(0, 2.2),
			]),
	...
	if (!isNomad())
	{
		...
		for (let i = 0; i < numPlayers; ++i)
			createAreasInAreas(
				...
				[
					new AndConstraint([
						avoidClasses(clPlayer, 12, clMountain, 3),
						borderClasses(clPlayer, 0, 37),
					]),
					new PassableMapAreaConstraint(),
				],
				1,
				100,
	...
	if (isNomad())
	{
		...
		{
			const connectedArea = floodFill(position, [
				avoidClasses(clMountain, 5, clForest, 0),
				stayClasses(clNonBordering, 0),
			]);
	...
	else
	{
		...
		const connectedArea = floodFill(pickRandom(playerPosition), [
			avoidClasses(clMountain, 5, clForest, 0),
			stayClasses(clNonBordering, 0),
		]);
		if (
			playerPosition.every((position) => connectedArea.contains(position))
		)
		{
			break;
		}
...
createAreas(
	new ChainPlacer(1, 10, randIntInclusive(1, 5), 1),
	new SmoothElevationPainter(ELEVATION_MODIFY, 0, 100),
	new AndConstraint([
		stayClasses(clMountain, 2),
		new SlopeConstraint(0.5, 1.7),
		new HeightConstraint(heightLand, heightLand + 60),
	]),
	scaleByMapSize(7, 35),
	20
);
...
createArea(new MapBoundsPlacer(), new TerrainPainter(tCliff), [
...
createArea(new MapBoundsPlacer(), new TerrainPainter(tMontainTop), [
	stayClasses(clMountain, 0),
	new SlopeConstraint(0.7, snowSlopeThreshold),
]);

createArea(new MapBoundsPlacer(), new TerrainPainter(tMontainPeak), [
	stayClasses(clMountain, 0),
...
createAreas(
	...
	[new TerrainPainter(pForestPineSnow[0]), new TileClassPainter(clForest)],
	new AndConstraint([
		stayClasses(clMountain, 2),
		new SlopeConstraint(0, 1.7),
		new HeightConstraint(heightLand, heightLand + 40),
	]),
	scaleByMapSize(3, 15),
	20
);
...
if (currentBiome() == "alpine_mountains/winter")
{
	...
	createAreas(
		new ChainPlacer(
			1,
			Math.floor(scaleByMapSize(6, 10)),
			Math.floor(scaleByMapSize(15, 30)),
			0.8
		),
		[
		...
		[
			avoidClasses(clPlayer, 30, clMountain, 2, clForest, 1),
			new HeightConstraint(0, heightLand + 7),
			new SlopeConstraint(0, 0.5),
		],
	...
	group = new SimpleGroup([
		new SimpleObject(
			aSnowMist,
			1,
			winterMistAmounts[winterEnvironment].maxObj,
			0.5,
			1.8,
			-Math.PI / 8,
			Math.PI / 8
	...
	createObjectGroups(
		...
		0,
		null,
		scaleByMapAreaAndPlayers(
			winterMistAmounts[winterEnvironment].base * randFloat(0.5, 1.5)
		),
		1
	);
	...
	group = new SimpleGroup([
		new SimpleObject(
			aSnowStorm,
			1,
			winterStormAmounts[winterEnvironment].maxObj,
			0.5,
			1.8,
			-Math.PI / 8,
			Math.PI / 8
	...
	createObjectGroups(
		...
		null,
		scaleByMapAreaAndPlayers(
			winterStormAmounts[winterEnvironment].base * randFloat(0.5, 1.5)
		),
		1
	);
...
group = new SimpleGroup(
	[new SimpleObject(oMetalSmall, 1, 4, 0, 1.5)],
	true,
	clMetal
);
createObjectGroupsByAreas(
	...
	0,
	[
		avoidClasses(clPlayer, 15, clForest, 1, clMetal, 8, clMountain, 1),
		borderClasses(clMountain, 0, 3),
		new SlopeConstraint(0, 1.2),
	],
	Math.floor(scaleByMapSize(8, 55) * randFloat(1, 1.5)),
	100,
	[mountainBorderArea]
);

group = new SimpleGroup(
	[new SimpleObject(oStoneLarge, 1, 1, 0, 1.5)],
	true,
	clRock
);
createObjectGroupsByAreas(
	...
	0,
	[
		avoidClasses(clPlayer, 15, clForest, 1, clMetal, 12, clRock, 12),
		borderClasses(clMountain, 0, 3),
	],
	Math.floor(scaleByMapSize(2, 10) * randFloat(1, 1.5)),
	100,
	[mountainBorderArea]
);

group = new SimpleGroup(
	[new SimpleObject(oStoneSmallA, 1, 4, 0, 1.5)],
	true,
	clRock
);
createObjectGroupsByAreas(
	...
	Math.floor(scaleByMapSize(4, 20) * randFloat(1, 1.5)),
	100,
	[mountainBorderArea]
);

group = new SimpleGroup(
	[new SimpleObject(oStoneSmallA, 1, 4, 0, 3)],
	true,
	clRock
);
createObjectGroups(
	...
	Math.floor(scaleByMapSize(1, 8) * randFloat(1, 2)),
	100
);

group = new SimpleGroup(
	[new SimpleObject(oStoneSmallB, 1, 4, 0, 3)],
	true,
	clRock
);
createObjectGroups(
...
if (!isNomad())
{
	...
	if (initialFoodAmount < 6) initialFoodAmount = 0;
	...
	for (let i = 0; i < numPlayers; ++i)
	{
		...
		while (remainingFood > 0)
		{
			...
			{
				remainingFood -= placeFoodForPlayer(
					pickRandom([oDeer, oGoat]),
					5,
					8,
					remainingFood,
					i
...
createObjectGroups(
	...
	[
		...
		borderClasses(clForest, 1, 1),
	],
	randFloat(0.5, 1) * scaleByMapSize(6, 30),
	20
);
...
createObjectGroups(
	...
	randFloat(0.2, 1) * scaleByMapSize(2, 10),
	5
);

g_Map.log("Creating goats");
group = new SimpleGroup([new SimpleObject(oGoat, 5, 7, 0, 4)], true, clFood);
createObjectGroups(
	group,
	0,
	avoidClasses(
	...
	randFloat(0.2, 1) * scaleByMapSize(3, 15),
	5
);

g_Map.log("Creating wolves");
group = new SimpleGroup([new SimpleObject(oWolf, 1, 3, 0, 4)], true, clWolf);
createObjectGroups(
	group,
	0,
	avoidClasses(
...
createStragglerTrees(
	[oPineSnow],
	new AndConstraint([
		stayClasses(clMountain, 1),
		new HeightConstraint(heightLand + 5, heightLand + 40),
...
createObjectGroups(
	...
	new AndConstraint([
		stayClasses(clMountain, 2),
		new SlopeConstraint(0, 1.7),
		new HeightConstraint(heightLand + 30, Infinity),
	]),
...
function scaleByMapAreaAndPlayers(
	base,
	countPlayers = false,
	minMapSize = 128
)
{
	const playerFactor = countPlayers ? numPlayers : 0;
	return (
		base *
        ((diskArea(mapSize / 2) - playerFactor * diskArea(20)) /
            diskArea(minMapSize / 2))
	);
}
...
function placeFoodForPlayer(type, min, max, remainingFood, areaId)
{
	...
	createObjectGroupsByAreas(
		...
		new AndConstraint([
			...
			borderClasses(clPlayer, 0, maxTileBound),
		]),
		1,
		400,
		[surroundingPlayersAreas[areaId]]
	);
...
function playerPlacementRandomScaled(playerIDs, constraints = undefined)
{
	...
	let playerMinDistSquared = Math.square(
		fractionToTiles(playerMinDistFraction)
	);
	...
	if (!area.getPoints().length) return undefined;
	...
	for (let i = 0; i < getNumPlayers(); ++i)
	{
		...
		if (
			locations.some(
				(loc) => loc.distanceToSquared(position) < playerMinDistSquared
			) ||
            position.distanceToSquared(mapCenter) >
                Math.square(mapCenter.x - borderDistance)
		)
		{
			--i;
			++attempts;
			...
			if (attempts > 500)
			{
				...
				if (resets % 25 == 0) playerMinDistSquared *= 0.95;

				// If we only pick bad locations, stop trying to place randomly
				if (resets == 500) return undefined;
			}
			continue;
		}
