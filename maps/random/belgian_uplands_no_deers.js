...
var heightRange = {
    "min": (MIN_HEIGHT * mapSize) / 8192,
    "max": (MAX_HEIGHT * mapSize) / 8192,
};
...
textueByHeight.push({
    "upperHeightLimit":
        heightRange.min + (3 / 3) * (heightSeaGroundAdjusted - heightRange.min),
    "terrain": "temp_mud_a",
});
...
terrains = ["temp_plants_bog|gaia/tree/bush_temperate"].concat(
    terrains,
    terrains
);
terrains = [
...
textueByHeight.push({
    "upperHeightLimit":
        heightSeaGroundAdjusted +
        (2 / 6) * (heightRange.max - heightSeaGroundAdjusted),
    "terrain": [
        "new_alpine_grass_b",
        "temp_grass_d",
        "new_alpine_grass_b",
...
textueByHeight.push({
    "upperHeightLimit":
        heightSeaGroundAdjusted +
        (3 / 6) * (heightRange.max - heightSeaGroundAdjusted),
    "terrain": [
        "temp_grass",
        "temp_grass_b",
        "temp_grass_c",
...
textueByHeight.push({
    "upperHeightLimit":
        heightSeaGroundAdjusted +
        (4 / 6) * (heightRange.max - heightSeaGroundAdjusted),
    "terrain": [
        "temp_grass",
        "temp_grass_b",
        "temp_grass_c",
...
for (var i = 0; i < numTerrains; i++) terrains.push("temp_grass_plants");
textueByHeight.push({
    "upperHeightLimit":
        heightSeaGroundAdjusted +
        (5 / 6) * (heightRange.max - heightSeaGroundAdjusted),
    "terrain": terrains,
});
...
textueByHeight.push({
    "upperHeightLimit":
        heightSeaGroundAdjusted +
        (6 / 6) * (heightRange.max - heightSeaGroundAdjusted),
    "terrain": [
...
while (true) {
    ...
    createArea(new MapBoundsPlacer(), new SmoothingPainter(2, 1, 20));
...
if (mapSize > 500) propDensity = 1 / 4;
else if (mapSize > 400) propDensity = 3 / 4;

for (let x = 0; x < mapSize; ++x)
    for (let y = 0; y < mapSize; ++y) {
        let position = new Vector2D(x, y);
        if (!g_Map.validHeight(position)) continue;

        var textureMinHeight = heightRange.min;
        for (var i = 0; i < textueByHeight.length; i++) {
            if (
                g_Map.getHeight(position) >= textureMinHeight &&
                g_Map.getHeight(position) <= textueByHeight[i].upperHeightLimit
            ) {
                ...
                if (i == 5) {
                    ...
                    if (randBool(propDensity / 800))
                        ...
                        template =
                            "actor|props/flora/decals_flowers_daisies.xml";
                    else if (randBool(propDensity / 200))
                        template =
                ...
                break;
            } else textureMinHeight = textueByHeight[i].upperHeightLimit;
        }
    }
...
if (isNomad())
    ...
    for (let i = 0; i < numPlayers; ++i) {
        ...
        for (let j = 1; j <= 4; ++j) {
            ...
            for (let k = 0; k < resourceCount; ++k) {
                let pos = Vector2D.sum([
                    ...
                    new Vector2D(k * resourceSpacing, 0).rotate(
                        -uAngle - Math.PI / 2
                    ),
                    new Vector2D(
                        -0.75 * resourceSpacing * Math.floor(resourceCount / 2),
                        0
        ...
        for (let index = 0; index < 2; index++) {
            let uAngle = BUILDING_ORIENTATION - (Math.PI * (2 - index)) / 2;
            for (let k = 0; k < chickenCount; ++k) {
                let pos = Vector2D.sum([
                    playerPosition[i],
                    new Vector2D(
                        chickenDistance + Math.floor(Math.random() * 10 + 1),
                        0
                    ).rotate(-uAngle),
                    new Vector2D(k + 1 * resourceSpacing, 0).rotate(
                        -uAngle - Math.PI / 2
                    ),
                    new Vector2D(
                        -0.55 * resourceSpacing * Math.floor(chickenCount / 2),
                        0
                    ).rotate(-uAngle - Math.PI / 2),
                ]);
